...
<x-app-layout>
    <h1 class="text-center mt-4">Modifier Votre Critique pour '{{$critique->anime?->titre}}'</h1>
    <p class="text-center custom-font-size2 italic text-danger">* Veuillez remplir les champs requis</p>
    <form method="post" action="{{route('critiques.update', $critique->id)}}">
        @csrf
        @method('PUT')
        <div class="row mt-3">
            <label for="titre" class="form-label">Titre<sup class="text-danger">*</sup></label>
            <input id="titre" name="titre" class="form-control" type="text" minlength="2" maxlength="50" value="{{$critique->titre}}" required>
            @error('titre')
        ...
        <div class="row mt-3">
            ...
            <select class="form-select" id="anime_id" name="anime_id" required>
                @foreach($animes as $anime)
                    <option value="{{$anime->id}}" @if($anime->id == $critique->anime_id)selected="selected"@endif>{{$anime->titre}}</option>
                @endforeach
            </select>
            @error('anime_id')
                <div class="text-danger text-bold mt-1">* {{$message}}</div>
        ...
        <div class="row mt-3">
            ...
            <select class="form-select" id="score" name="score" required>
                ...
                @for($i = 1; $i<=10; $i++)
                    <option value="{{$i}}" @if($i == $critique->score)selected="selected"@endif>{{$i}}</option>
                @endfor
            </select>
            @error('score')
                <div class="text-danger text-bold mt-1">* {{$message}}</div>
        ...
        <div class="row mt-3">
            ...
            <textarea id="texte" name="texte" class="form-control" type="text" minlength="10" maxlength="1000" rows="5" cols="50" required>{{$critique->texte}}</textarea>
            @error('texte')
                <div class="text-danger text-bold mt-1">* {{$message}}</div>
            @enderror
        </div>
